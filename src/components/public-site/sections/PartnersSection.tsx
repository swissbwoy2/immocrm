...
function PartnerLogo({ partner }: { partner: Partner }) {
  const img = <img src={partner.logo} alt={partner.name} className="h-10 md:h-12 w-auto object-contain grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-500" />;
  if (partner.href) {
    return <a href={partner.href} target="_blank" rel="noopener noreferrer" className="group flex items-center justify-center p-4 rounded-xl hover:bg-background/60 transition-all duration-300" title={partner.name}>{img}</a>;
  }
  return <div className="group flex items-center justify-center p-4 rounded-xl hover:bg-background/60 transition-all duration-300" title={partner.name}>{img}</div>;
}

export function PartnersSection() {
  return (
    <section id="partenaires" className="py-24 md:py-32 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-12 animate-fade-in">
          <div className="inline-flex items-center gap-2 bg-background/80 rounded-full px-4 py-2 mb-4 border border-border/40">
            <Handshake className="h-4 w-4 text-primary" /><span className="text-sm font-medium text-primary">Écosystème de confiance</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-3">Nos partenaires de <span className="text-primary">confiance</span></h2>
          <p className="text-muted-foreground max-w-xl mx-auto">Nous collaborons avec les acteurs majeurs de l'immobilier et de l'assurance en Suisse</p>
        </div>
