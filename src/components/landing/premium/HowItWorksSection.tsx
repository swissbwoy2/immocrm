...
export function HowItWorksSection() {
  return (
    <section id="comment-ca-marche" className="py-16 md:py-24 bg-background">
        ...
        <div className="max-w-2xl mx-auto space-y-0">
          {steps.map((s, i) => (
            <div key={i} className="flex gap-6 relative">
              {/* Vertical line */}
              {i < steps.length - 1 && (
                <div className="absolute left-6 top-14 bottom-0 w-px bg-primary/20" />
              )}
              {/* Number circle */}
              <div className="flex-shrink-0 w-12 h-12 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-sm font-bold z-10">
                {s.num}
              </div>
              <div className="pb-10">
