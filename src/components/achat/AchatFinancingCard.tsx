...
const STATUT_BANCAIRE_LABEL: Record<string, { label: string; color: string }> = {
  ...
  validation_orale: { label: 'Validation orale reçue', color: 'bg-sky-100 text-sky-700' },
    ...
    return (
      <Card className="p-6 border-sky-100">
        <div className="text-center py-8">
          <Banknote className="h-10 w-10 text-sky-600 mx-auto mb-3 opacity-60" />
  ...
  return (
    <Card className="p-6 border-sky-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
        ...
        <Metric icon={Wallet} label="Fonds propres totaux" value={formatCHF(computed.fondsPropresTotal)} sub={`Dont LPP/EPL : ${formatCHF(computed.fondsPropresLpp)}`} color="text-sky-600" />
        <Metric icon={Building2} label="Prix cible" value={formatCHF(computed.prixCible)} sub={`Apport requis (${settings.fonds_propres_min}%) : ${formatCHF(computed.fondsPropresRequis)}`} color="text-indigo-600" />
      ...
      <div className="rounded-xl border border-sky-100 bg-sky-50/40 p-4 mb-4">
        <div className="text-xs uppercase tracking-wider text-sky-700 font-semibold mb-3">Charges théoriques</div>
