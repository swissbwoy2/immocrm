...
const STATUT_LABEL: Record<string, { label: string; color: string }> = {
  ...
  rdv_planifie:       { label: 'RDV planifié', color: 'bg-sky-100 text-sky-700' },
      ...
      <div className="space-y-3 text-sm">
        ...
        {notary.notaire_email && (
          <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <a href={`mailto:${notary.notaire_email}`} className="hover:text-sky-600">{notary.notaire_email}</a>
          </div>
        )}
        {notary.notaire_telephone && (
          <div className="flex items-center gap-2 text-muted-foreground">
            <Phone className="h-4 w-4" />
            <a href={`tel:${notary.notaire_telephone}`} className="hover:text-sky-600">{notary.notaire_telephone}</a>
          </div>
        )}
