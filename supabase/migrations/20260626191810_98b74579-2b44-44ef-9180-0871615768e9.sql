ALTER TABLE public.clients DROP CONSTRAINT IF EXISTS clients_journey_type_check;
ALTER TABLE public.clients ADD CONSTRAINT clients_journey_type_check CHECK (journey_type = ANY (ARRAY['housing_search'::text, 'property_reletting'::text, 'mixed'::text, 'purchase_search'::text]));

WITH buyer_clients AS (
  SELECT c.id, c.user_id, c.agent_id
  FROM public.clients c
  LEFT JOIN public.profiles p ON p.id = c.user_id
  WHERE lower(coalesce(c.type_recherche, '')) IN ('acheter', 'achat', 'purchase', 'purchase_search')
  ...
  SET type_recherche = 'Acheter',
      ...
      priorite = COALESCE(c.priorite, 'haute'),
      statut = COALESCE(c.statut, 'en_attente')
  ...
  INSERT INTO public.purchase_projects (
    ...
    montant_mandat,
    montant_acompte,
    duree_progression_jours,
    date_debut_progression
  )
  SELECT n.id,
         n.user_id,
         n.agent_id,
  ...
  SELECT pp.id, pp.client_id, pp.user_id
  FROM public.purchase_projects pp
  JOIN normalized_clients n ON pp.client_id = n.id OR (pp.user_id IS NOT NULL AND pp.user_id = n.user_id)
)
UPDATE public.purchase_projects pp
SET user_id = COALESCE(pp.user_id, ap.user_id),
    montant_mandat = 4999,
...
WHERE c.journey_type = 'purchase_search'
  AND NOT EXISTS (
    SELECT 1 FROM public.purchase_financing_profiles pf WHERE pf.project_id = pp.id
  );
