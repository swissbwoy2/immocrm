DROP FUNCTION IF EXISTS public.get_public_showcase_annonces();

CREATE OR REPLACE FUNCTION public.get_public_showcase_annonces()
 RETURNS TABLE(id uuid, titre text, type_bien text, adresse text, prix numeric, pieces numeric, surface numeric, etage text, lien_annonce text, medias_galerie jsonb, date_envoi timestamp with time zone, type_transaction text, est_mise_en_avant boolean)
 LANGUAGE sql
...
AS $function$
  SELECT a.id,
         ...
         a.type_transaction,
         COALESCE(a.est_mise_en_avant, false) AS est_mise_en_avant
  ...
  ORDER BY COALESCE(a.est_mise_en_avant, false) DESC,
           a.mise_en_avant_rang ASC NULLS LAST,
           a.mise_en_avant_depuis DESC NULLS LAST,
           COALESCE(a.date_publication, a.created_at) DESC
  LIMIT 20;
$function$;
