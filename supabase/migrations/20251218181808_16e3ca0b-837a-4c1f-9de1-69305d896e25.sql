-- Update notify_on_new_message to include conversationId in link
    ...
    IF NEW.sender_type = 'agent' THEN
      ...
      IF v_conversation.client_id IS NOT NULL THEN
        ...
        IF v_recipient_user_id IS NOT NULL THEN
          PERFORM create_notification(
            ...
            '/client/messagerie?conversationId=' || NEW.conversation_id,
      ...
      LOOP
        PERFORM create_notification(
          ...
          '/agent/messagerie?conversationId=' || NEW.conversation_id,
          jsonb_build_object('conversation_id', NEW.conversation_id::text)
        );
      END LOOP;
    ...
    ELSIF NEW.sender_type = 'client' THEN
      ...
      LOOP
        PERFORM create_notification(
          ...
          'Message de ' || COALESCE(v_sender_name, 'un client'),
          '/admin/messagerie?conversationId=' || NEW.conversation_id,
          jsonb_build_object('conversation_id', NEW.conversation_id::text)
        );
      END LOOP;
    END IF;
...
-- Update notify_on_new_offer to include offre_id in link
  ...
  IF NEW.client_id IS NOT NULL THEN
    ...
    IF v_client_user_id IS NOT NULL THEN
      PERFORM create_notification(
        ...
        '/client/offres-recues?offreId=' || NEW.id,
    ...
    LOOP
      PERFORM create_notification(
        ...
        '/admin/offres-envoyees?offreId=' || NEW.id,
...
-- Update notify_on_new_visit to include visite_id in link
...
-- Update notify_on_candidature_status_change to include candidature_id in links
  ...
  IF OLD.statut IS DISTINCT FROM NEW.statut THEN
    ...
    CASE NEW.statut
      WHEN 'acceptee' THEN
        ...
        v_notification_link := '/client/mes-candidatures?candidatureId=' || NEW.id;
      ...
      WHEN 'bail_conclu' THEN
        ...
        IF v_agent_user_id IS NOT NULL THEN
          PERFORM create_notification(
            ...
            '/agent/candidatures?candidatureId=' || NEW.id,
    ...
    IF v_admin_title IS NOT NULL THEN
      ...
      LOOP
        PERFORM create_notification(
          ...
          v_admin_title,
          v_admin_message,
          '/admin/candidatures?candidatureId=' || NEW.id,
...
-- Update notify_on_visite_status_change to include visite_id in links
